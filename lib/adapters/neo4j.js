...
Neo4j.prototype.create = function create(model, data, callback) {
    ...
    var tableModel = this._models[ table ].model;
    data.nodeType = table;
        ...
        node.index(table, 'id', node.id, function (err) {
            ...
            this.updateIndexes(model, node, function (err) {
                ...
                if (tableModel.relations) {
                    var relationsToProcess = 0;
                    for(var name in tableModel.relations ) {
                        relationsToProcess++;
                    }

                    for(var name in tableModel.relations ) {
                        var relation = tableModel.relations[ name ];

                        switch(relation.type) {
                            case 'belongsTo':
                                tableModel.createRelationshipTo( node.id, node.data[ relation.keyFrom ], 'belongsTo', {}, function(err) {
                                    if (err) return callback(err);
                                    
                                    callMeWhenImDone(--relationsToProcess);
                                });
                            break;
                            default:
                                callMeWhenImDone(--relationsToProcess);
                    ...
                    function callMeWhenImDone(toProcess) {
                        if (toProcess === 0) 
